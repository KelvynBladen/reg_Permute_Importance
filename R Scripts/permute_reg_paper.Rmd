...
```{r, eval=TRUE}
set.seed(123)
permute_refit <- vector(length = 6)
drop_refit <- vector(length = 6)
permute_valid <- vector(length = 6)
drop_valid <- vector(length = 6)
mrep <- 1
for (j in seq_len(mrep)) {
  ...
  imp1 <- pmax(imp, 0)
  simp <- sqrt(imp1)
  permute_refit <- permute_refit + simp / mrep
  
  impv1 <- pmax(impv, 0)
  simpv <- sqrt(impv1)
  permute_valid <- permute_valid + simpv / mrep
  ...
  drop_refit <- drop_refit + drop_simp / mrep
  
  drop_impv1 <- pmax(drop_impv, 0)
  drop_simpv <- sqrt(drop_impv1)
  drop_valid <- drop_valid + drop_simpv / mrep
}

both <- as.data.frame(cbind(Coef = 5:0,
                            Estimate = s$coefficients[-1, 1],
                            permute_valid, drop_valid,
                            t_value = s$coefficients[-1, 3],
                            permute_refit, drop_refit))
# why do these match t rather than t^2? Because I square rooted them
ggpairs(both)
#cor(both)
both

#ggsave("ind.pdf", ggpairs(both), dpi = 2400, width = 7, height = 7)
```
...
permute_refit <- vector(length = 8)
drop_refit <- vector(length = 8)

permute_valid <- vector(length = 8)
drop_valid <- vector(length = 8)

permute_refit_rf <- vector(length = 8)
drop_refit_rf <- vector(length = 8)

permute_valid_rf <- vector(length = 8)
drop_valid_rf <- vector(length = 8)
...
for (j in seq_len(mrep)) {
  ...
  for (i in seq_len(8)) {
    ...
    pp_valid = predict(reg_full, dfv_new)
    sp_valid = (pp_valid-dfv_new$yv)
    new_mv = mean((pp_valid-dfv_new$yv)^2)
    impv[i] <- (new_mv - mv)
    
    pr_valid = predict(rf, dfv_new)
    new_rfmv = mean((pr_valid-dfv_new$yv)^2)
  ...
  simprf <- sqrt(imprf1)
  permute_refit_rf <- permute_refit + simprf / mrep
  
  imprfv1 <- pmax(imprfv, 0)
  simprfv <- sqrt(imprfv1)
  permute_valid_rf <- permute_valid + simprfv / mrep
  ...
  for (i in seq_len(8)) {
    ...
    dp_valid = predict(reg_full, dfv_new)
    sd_valid = (dp_valid-dfv_new$yv)
    new_mv = mean((dp_valid-dfv_new$yv)^2)
  ...
  simp <- sqrt(imp1)
  drop_refit <- drop_refit + simp / mrep
  ...
  simprf <- sqrt(imprf1)
  drop_refit_rf <- drop_refit + simprf / mrep
  
  drop_imprfv1 <- pmax(drop_imprfv, 0)
  drop_simprfv <- sqrt(drop_imprfv1)
  drop_valid_rf <- drop_valid + drop_simprfv / mrep
}

# df = as.data.frame(cbind(pp_res, dp_res, pp_valid, dp_valid))
# ggpairs(df)
# df = as.data.frame(cbind(sp_res, sd_res, sp_valid, sd_valid))
# ggpairs(df)
sc
rf$rsq[500]

sdf <- as.data.frame(cbind(coef = c(5, 5, 2, 0, 5, 5, 2, 0),
                           Estimate = sc$coefficients[2:9, 1],
                           permute_valid, drop_valid,
                           t_value = sc$coefficients[2:9, 3],
                           permute_refit, drop_refit))
sdf
ggpairs(sdf)
#ggsave("stroblf.pdf", ggpairs(sdf), dpi = 2400, width = 7, height = 7)
...
sdf1 <- as.data.frame(cbind(coef = c(5, 5, 2, 0, 5, 5, 2, 0),
                           Estimate = sc$coefficients[2:9, 1],
                           permute_valid = permute_valid_rf, 
                           drop_valid = drop_valid_rf,
                           t_value = sc$coefficients[2:9, 3],
                           rf_oob = imp_df$`%IncMSE`,
                           permute_refit = permute_refit_rf, 
                           drop_refit = drop_refit_rf))
sdf1
ggpairs(sdf1)


#ggsave("stroblrf.jpg", ggpairs(sdf1), dpi = 2400, width = 7, height = 7)
```
...
# drop_refit <- vector(length = 8)
# 
# permute_refit_old <- vector(length = 8)
# drop_refit_old <- vector(length = 8)
...
for (j in seq_len(mrep)) {
  ...
  # imp1 <- pmax(impo, 0)
  # simp <- sqrt(imp1)
  # permute_refit_old <- permute_refit_old + simp / mrep
  ...
  # simp <- sqrt(imp1)
  # drop_refit_old <- drop_refit_old + simp / mrep
...
#cor(sdf)
sdf
Ind = as.factor(rep(c("No", "Yes"), each = 4))

ggpairs(sdf, aes(col = Ind))


g <- ggpairs(sdf, legend = c(2,1),
             lower = list(mapping = aes(color = Ind)),
...
#ggsave("stroblsd5.pdf", g, dpi = 2400, width = 7, height = 7)

# sdfb <- as.data.frame(cbind(coef = c(5, 5, 2, 0, 5, 5, 2, 0),
#              sc$coefficients[2:9, c(1, 3)], 
#              # permute_refit, drop_refit,
#              # permute_refit_old, drop_refit_old, 
#              permute_refit, drop_refit,
#              permute_valid, drop_valid))
# ggpairs(sdfb)
...
# sdd <- as.data.frame(cbind(coef = c(5, 5, 2, 0, 5, 5, 2, 0),
#              sc$coefficients[2:9, c(1, 3)], 
#              permute_refit,
#              permute_valid, rf_permute = imp_df$`%IncMSE`))
#sdd
#cor(sdd)
#data.frame(v1 = cor(sdd)[1,])
#ggpairs(sdd)
#ggsave("stroblrf.jpg", ggpairs(sdd), dpi = 2400, width = 7, height = 7)

# sddb <- as.data.frame(cbind(coef = c(5, 5, 2, 0, 5, 5, 2, 0),
#              sc$coefficients[2:9, c(1, 3)], 
#              # permute_refit, 
#              # permute_refit_old, 
#              permute_refit,
#              permute_valid, imp_df))
# ggpairs(sddb)
...
# sdd1 <- as.data.frame(cbind(coef = c(5, 5, 2, 0, 5, 5, 2, 0),
#              sc$coefficients[2:9, c(1, 3)], permute_refit,
#              permute_valid, RF_permute = imp_df$`%IncMSE`,
...
#ggpairs(sdd1)
# ggpairs(sdd1)
# ggsave("stroblsdrf.jpg", ggpairs(sdd1), dpi = 2400, width = 7, height = 7)
```
