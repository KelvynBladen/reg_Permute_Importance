...
```{r, eval=TRUE}
set.seed(123)
permute_LOCO <- vector(length = 6)
drop_LOCO <- vector(length = 6)
permute_PaP <- vector(length = 6)
drop_PaP <- vector(length = 6)
mrep <- 1
for (j in seq_len(mrep)) {
  ...
  imp1 <- pmax(imp, 0)
  simp <- sqrt(imp1)
  permute_LOCO <- permute_LOCO + simp / mrep
  
  impv1 <- pmax(impv, 0)
  simpv <- sqrt(impv1)
  permute_PaP <- permute_PaP + simpv / mrep
...
permute_LOCO <- vector(length = 8)
drop_LOCO <- vector(length = 8)

permute_PaP <- vector(length = 8)
drop_PaP <- vector(length = 8)

permute_LOCO_rf <- vector(length = 8)
drop_LOCO_rf <- vector(length = 8)

permute_PaP_rf <- vector(length = 8)
drop_PaP_rf <- vector(length = 8)
...
for (j in seq_len(mrep)) {
  ...
  simprf <- sqrt(imprf1)
  permute_LOCO_rf <- permute_LOCO + simprf / mrep
  
  imprfv1 <- pmax(imprfv, 0)
  simprfv <- sqrt(imprfv1)
  permute_PaP_rf <- permute_PaP + simprfv / mrep
  ...
  drop_PaP_rf <- drop_PaP + drop_simprfv / mrep
}
...
sdf1 <- as.data.frame(cbind(coef = c(5, 5, 2, 0, 5, 5, 2, 0),
                           Estimate = sc$coefficients[2:9, 1],
                           permute_PaP = permute_PaP_rf, 
                           drop_PaP = drop_PaP_rf,
                           t_value = sc$coefficients[2:9, 3],
                           OOB_PaP = imp_df$`%IncMSE`,
                           permute_LOCO = permute_LOCO_rf, 
...
# drop_LOCO <- vector(length = 8)
# 
# permute_LOCO_old <- vector(length = 8)
...
for (j in seq_len(mrep)) {
  ...
  # imp1 <- pmax(impo, 0)
  # simp <- sqrt(imp1)
  # permute_LOCO_old <- permute_LOCO_old + simp / mrep
...
#              # permute_LOCO, drop_LOCO,
#              # permute_LOCO_old, drop_LOCO_old, 
#              permute_LOCO, drop_LOCO,
#              permute_PaP, drop_PaP))
# ggpairs(sdfb)
...
#              sc$coefficients[2:9, c(1, 3)], 
#              permute_LOCO,
#              permute_PaP, rf_permute = imp_df$`%IncMSE`))
#sdd
#cor(sdd)
...
#              sc$coefficients[2:9, c(1, 3)], permute_LOCO,
#              permute_PaP, RF_permute = imp_df$`%IncMSE`,
