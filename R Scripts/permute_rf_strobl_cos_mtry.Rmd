---
title: "permute_rf_strobl_cos_mtry"
...
for (j in seq_len(mrep)) {
    ...
    strobl = (strobl - 0.5)*8*pi
  
    y <- 5 * cos(strobl[, 1]) + 5 * cos(strobl[, 2]) + 2 * cos(strobl[, 3]) +
      5 * cos(strobl[, 5]) + 5 * cos(strobl[, 6]) + 2 * cos(strobl[, 7])
    strobl <- data.frame(cbind(strobl, y))
  
    dfv <- MASS::mvrnorm(n_size, mu = rep(0, 12), Sigma = sig)
    dfv = apply(dfv, 2, pnorm)
    dfv = (dfv - 0.5)*8*pi
    y <- 5 * cos(dfv[, 1]) + 5 * cos(dfv[, 2]) + 2 * cos(dfv[, 3]) +
      5 * cos(dfv[, 5]) + 5 * cos(dfv[, 6]) + 2 * cos(dfv[, 7])
...
library(patchwork)
gr + gp + gd + gv
ggsave("xcos_all_zoom.pdf", plot = gr + gp + gd + gv, dpi = 2400,
       width = 9, height = 9)
```
...
library(patchwork)
gr + gp + gd + gv
ggsave("xcos_all.pdf", plot = gr + gp + gd + gv, dpi = 2400,
       width = 9, height = 9)
